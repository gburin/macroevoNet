...
#### Using body mass for PCA to analyze ecomorphospace occupation
...
## Using residuals from regression between richness and disparity to test for relationship with ClimPC's

amp.resid <- summary(lm(disp.per.net1$amplitude ~ net.metadata$n.bird.species[match(disp.per.net1$network, net.metadata$Network)]))$residuals

mnnd.resid <- summary(lm(disp.per.net1$mnnd ~ net.metadata$n.bird.species[match(disp.per.net1$network, net.metadata$Network)]))$residuals

avpdist.resid <- summary(lm(disp.per.net1$av.pdist ~ net.metadata$n.bird.species[match(disp.per.net1$network, net.metadata$Network)]))$residuals

resid.plot <- data.frame(
	amp.resid = amp.resid,
	mnnd.resid = mnnd.resid,
	avpdist.resid = avpdist.resid,
	clim.pc1 = disp.per.net1$clim.pc1,
	clim.pc2 = disp.per.net1$clim.pc2
	)
